defmodule AlgoThinkWeb.Button do
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "rounded-lg bg-zinc-900 hover:bg-zinc-700 py-2 px-3 flex flex-row items-center gap-2 transition-colors",
        "text-sm font-semibold leading-6 text-white active:text-white/80",
        "phx-submit-loading:spinner-visible phx-submit-loading:pointer-events-none",
        if @disabled do "!bg-gray-300 !text-gray-500 pointer-events-none" end,
        @class
      ]}
      {@rest}
    >
      ...
      <%= if @disabled do %>
        <MaterialIcons.check class="relative p-1 fill-gray-800 rounded-full bg-green-400" size={24} />
      <% end %>
    </button>
    """
  end
